...
async def handle_file_upload(file: UploadFile = File(...)):
    ...
    return {"info": f"File '{UPLOADED_FILENAME}' saved at '{file_location}'"}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while reload:
        while change_file and no_file_uploaded:
            await websocket.send_text(
                "Please upload your file. (xlsx, pdf, csv supported only)"
            )
            ...
            if filename_data.startswith("uploaded:"):
                ...
                await websocket.send_text(f"uploading: {UPLOADED_FILENAME}...")
                # Call the function to extract information using Shazam
                # info = shazam_search.bgr_image(UPLOADED_FILENAME)
                # if info != 0:
                #     title = info[0]
                #     artist = info[1]
                #     background_image_url = info[2]

                # no_file_uploaded = False
        ...
        if data == "1":
            await websocket.send_text("You selected AAAA.")
            await asyncio.sleep(1)
            await websocket.send_text("Wait for AAAA...")
            # spl = run_spleeter.run_spleeter(UPLOADED_FILENAME)
            # vrm = run_vocal_remover.run_vocal_remover(UPLOADED_FILENAME)
            ...
            await asyncio.sleep(0.3)
            path = f"uploaded_files/{UPLOADED_FILENAME}"
        ...
        if continue_operation.lower() == "no" or continue_operation.lower() == "n":
            ...
            if same_operation.lower() == "no" or same_operation.lower() == "n":
                ...
                change_file = True
                no_file_uploaded = True
                # run_spleeter.delete_uploaded(UPLOADED_FILENAME)
                UPLOADED_FILENAME = ""
